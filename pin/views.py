    ...
    if request.is_ajax():
        if latest_items.exists():
            return render_to_response('pin/_items.html', 
                              {'latest_items': latest_items,'pin_form':form, 'offset':0},
...
def popular(request):
    ...
    if latest_items.has_next(): 
       next_page_number=latest_items.next_page_number()
    else:
       next_page_number=0
    form = PinForm()
    ...
    if request.is_ajax():
        return render_to_response('pin/_items.html', 
                              {'latest_items': latest_items,'pin_form':form,'offset':next_page_number},
                              context_instance=RequestContext(request))
        
    else:
        return render_to_response('pin/home.html', 
                              {'latest_items': latest_items, 'offset':next_page_number},
                              context_instance=RequestContext(request))
...
def user(request, user_id):
    ...
    posts=map((lambda a : a.post.stock_code),Likes.objects.filter(user=user))
    latest_items = Post.objects.filter(stock_code__in=posts).order_by('rating')
    ...
    if request.is_ajax():
        ...
        if latest_items.exists():
           ...
           l_items=latest_items
        else:
           follow_status = Follow.objects.filter(follower=request.user.id).count()
           l_items=[{'user':user,'user_id':user_id,'image':None}]
        
        return render_to_response('pin/user.html', 
                              {'latest_items': l_items, 'follow_status':follow_status,
                               'profile':profile,
                               'cur_user':user, 'offset':0},
                              context_instance=RequestContext(request))
...
def following(request):
    ...
    latest_items = Post.objects.filter(stock_code__in=idis).all()
    
    objects = dict([(obj.stock_code, obj) for obj in latest_items])
    ...
    if request.is_ajax():
        if latest_items.exists():
            return render_to_response('pin/_items.html', 
                              {'latest_items': sorted_objects,'pin_form':form, 'offset': 0},
                              context_instance=RequestContext(request))
        else:
            return HttpResponse(0)
    else:
        return render_to_response('pin/home.html', 
                              {'latest_items': sorted_objects, 'offset': 0},
                              context_instance=RequestContext(request))
...
def item(request, item_id):
    ...
    item = get_object_or_404(Post.objects.filter(stock_code=item_id)[:1])
    
    latest_items = Post.objects.exclude(stock_code=item_id).order_by('rating')[:20]
    ...
    if request.is_ajax():
        ...
        return render_to_response('pin/item.html', 
                              {'item_inner': item, 'latest_items': latest_items, 'likes':likes, 'follow_status':follow_status, 'offset':0},
                              context_instance=RequestContext(request))
...
def a_sendurl(request):
    if request.method == "POST":
        ...
        if url == '':
            return HttpResponse(0)
        print url
        url="http://image.sinajs.cn/newchart/daily/n/%s.gif" % url
...
def comment_posted(request):
    if request.GET['c']:
        ...
        entry = Post.objects.get(stock_code=comment.object_pk) #C
        if entry:
...
def delete(request, item_id):
    try:
        ...
        if post.user == request.user:
            likes = Likes.objects.filter(post_id=post.stock_code)
            for like in likes:
                like.delete()
...
def like(request, item_id):
    ...
    try:
        ...
        Post.objects.filter(stock_code=item_id).update(like=current_like)
        ...
        if request.is_ajax():
            ...
            return HttpResponseRedirect(reverse('pin-item', args=[post.stock_code]))
